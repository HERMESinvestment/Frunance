...
def fig_velas(df_p, nombre, freq, color="#00e676"):
    ...
    ser=pd.to_numeric(df[cp],errors="coerce")
    oh=ser.resample(freq).agg(["first","max","min","last"])
    oh.columns=["open","high","low","close"]; oh.dropna(inplace=True)
    if oh.empty: return None
    for cr,co in [("precio_min","low"),("precio_max","high")]:
        if cr in df.columns:
            oh[co]=pd.to_numeric(df[cr],errors="coerce").resample(freq).agg("min" if co=="low" else "max")
    tv="volumen" in df.columns and pd.to_numeric(df["volumen"],errors="coerce").notna().any()
    fig=make_subplots(rows=2 if tv else 1,cols=1,shared_xaxes=True,
        row_heights=[0.72,0.28] if tv else [1],vertical_spacing=0.04)
    dn="#ff1744"
    fig.add_trace(go.Candlestick(x=oh.index,open=oh["open"],high=oh["high"],
        low=oh["low"],close=oh["close"],name=nombre,
        increasing_line_color=color,increasing_fillcolor=color,
        decreasing_line_color=dn,decreasing_fillcolor=dn),row=1,col=1)
    if len(oh)>=4:
        fig.add_trace(go.Scatter(x=oh.index,y=oh["close"].rolling(4).mean(),
            name="Media móvil",line=dict(color="#FFD600",width=1.8,dash="dot")),row=1,col=1)
    if tv:
        vol=pd.to_numeric(df["volumen"],errors="coerce").resample(freq).sum()
        clrs=[color if c>=o else dn for c,o in zip(oh["close"],oh["open"])]
        fig.add_trace(go.Bar(x=vol.index,y=vol.values,name="Volumen",
            marker_color=clrs,opacity=0.65),row=2,col=1)
    fig.update_layout(title=dict(text=f"📈  {nombre}",font=dict(size=18,family=FONT)),
        xaxis_rangeslider_visible=False,template="plotly_dark",height=520,
        yaxis_title="Precio (COP)",legend=dict(orientation="h",y=1.06,x=0),
...
def fig_comparar_mercados(df_all, producto, freq):
    ...
    for merc in mercados:
        ...
        if not cp: continue
        ser=pd.to_numeric(dm[cp],errors="coerce")
        oh=ser.resample(freq).agg(["first","max","min","last"])
        oh.columns=["open","high","low","close"]; oh.dropna(inplace=True)
...
# ══════════════ MAIN ═════════════════════════
st.markdown('<h1 style="letter-spacing:0.15em;font-size:2.2rem;">🌿 FRUNANCE</h1>',unsafe_allow_html=True)
st.caption("Plataforma de inteligencia de mercados de alimentos · Colombia")
...
with t1:
    ...
    ps=ca.selectbox("Producto",prods,key="t1p")
    ag=cb.selectbox("Agrupar",["Semana","Mes","Día"],key="t1ag")
    mf=cc.selectbox("Mercado",["Todos"]+sorted(df_all["mercado"].unique().tolist()),key="t1m")
    freq={"Semana":"W","Mes":"ME","Día":"D"}[ag]
    ...
    if len(dfp)<2:
        ...
        if crp:
            ...
            pa=dfp[dfp["fecha"]==dfp["fecha"].max()][crp].values
            pa=pa[0] if len(pa) else None
            pant=dfp[dfp["fecha"]<dfp["fecha"].max()][crp].dropna().values
            m1,m2,m3,m4=st.columns(4)
            m1.metric("Precio actual",fmtcop(pa) if pa else "N/A")
            if pa and len(pant):
                cam=pa-pant[-1]; pct=cam/pant[-1]*100
                m2.metric("Cambio vs ayer",fmtcop(cam),f"{pct:+.1f}%")
            m3.metric("Máximo histórico",fmtcop(dfp[crp].max()))
            m4.metric("Mínimo histórico",fmtcop(dfp[crp].min()))
...
with t7:
    ...
    if ams:
        ...
        if pdfs:
            ...
            pdfs[0].seek(0)
            if tbs:
                tbs_opc=[f"Tabla {i+1} ({len(tbs[i])} filas)" for i in range(len(tbs))]
                tbs_sel=st.selectbox("Tabla",tbs_opc,key="mpt")
                itp=tbs_opc.index(tbs_sel)
                t7=tbs[itp]
                # Limpiar encabezados None y duplicados
                headers7 = [str(h) if h is not None else f"col_{i}" for i,h in enumerate(t7[0])]
                # Hacer únicos los encabezados duplicados
                seen={}
                unique_h=[]
                for h in headers7:
                    if h in seen:
                        seen[h]+=1
                        unique_h.append(f"{h}_{seen[h]}")
                    else:
                        seen[h]=0
                        unique_h.append(h)
                try:
                    df7=pd.DataFrame(
                        [[str(c) if c is not None else "" for c in row] for row in t7[1:]],
                        columns=unique_h)
                    st.dataframe(df7.head(3),use_container_width=True)
                except Exception as e:
                    st.warning(f"Vista previa no disponible para esta tabla: {e}")
                    df7=pd.DataFrame(columns=unique_h)
                c7=list(df7.columns); o7=[NINGUNA]+c7
                p71,p72=st.columns(2)
                cpdf={"ti":itp,
                    "prod":p71.selectbox("PRODUCTO *",c7,key="mprod"),
            ...
            for i,f in enumerate(ams):
                ...
                try:
                    if f.name.lower().endswith(".pdf") and cpdf:
                        r=parse_pdf(f,cpdf["ti"],cpdf["prod"],cpdf["min"],cpdf["max"],cpdf["prom"],cpdf["vol"]); fk="pdf"
                    else:
