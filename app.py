...
with t1:
    ...
    ag=cb.selectbox("Agrupar",["Última semana","Semana","Mes","Día"],key="t1ag")
        ...
        if pa:
            ...
            if len(pant):
                ...
                st.markdown(
                    ...
                    f'<b>Último precio ({fecha_ult}):</b> '
                    f'<span style="font-size:1.4rem;font-weight:800;">{fmtcop(pa)}</span>'
                    f'&nbsp;&nbsp;<span style="color:{color_arr};font-size:1rem;">'
                    f'{arrow} {fmtcop(abs(cam))} ({pct:+.1f}%)</span></div>',
                    unsafe_allow_html=True)
            else:
                st.markdown(
                    f'<div style="font-size:1.1rem;padding:8px 0 4px 0;">'
                    f'<b>Último precio ({fecha_ult}):</b> '
                    f'<span style="font-size:1.4rem;font-weight:800;">{fmtcop(pa)}</span></div>',
                    unsafe_allow_html=True)
