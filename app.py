...
with st.sidebar:
    ...
    st.markdown("""
    <style>
    .flag-btn { display:flex; gap:10px; margin-bottom:8px; }
    .flag-btn a {
        flex:1; text-align:center; text-decoration:none;
        font-size:1.8rem; padding:6px; border-radius:8px;
        background:#1a1a2e; border:2px solid transparent;
        transition:border 0.2s;
    }
    .flag-btn a:hover { border:2px solid #00e676; }
    </style>
    """, unsafe_allow_html=True)

    lang_sel = st.radio(
        "Idioma",
        options=["🇨🇴  Español", "🇨🇼  Papiamento"],
        index=0 if st.session_state.get("idioma","es")=="es" else 1,
        horizontal=True,
        label_visibility="collapsed"
    )
    nuevo_idioma = "es" if "Español" in lang_sel else "pap"
    if nuevo_idioma != st.session_state.get("idioma","es"):
