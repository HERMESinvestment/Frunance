...
with st.sidebar:
    ...
    if pf:
        ...
        if not tabs: st.error("No se encontraron tablas.")
            ...
            it=st.selectbox("Tabla",range(len(tabs)),
                format_func=lambda i:f"Tabla {i+1} ({len(tabs[i])} filas)")
    ...
    if historico_ya_existe():
        ...
        if st.button("⚡ Cargar demo",use_container_width=True,type="primary"):
            ...
            st.success(f"✔ {n:,} registros"); df_all=cargar_datos(); st.rerun()
...
with t7:
    ...
    if ams:
        ...
        if pdfs:
            ...
            pdfs[0].seek(0)
            if tbs:
                itp=st.selectbox("Tabla",range(len(tbs)),
                    format_func=lambda i:f"Tabla {i+1} ({len(tbs[i])} filas)",key="mpt")
                t7=tbs[itp]; df7=pd.DataFrame(t7[1:],columns=t7[0]).astype(str)
        ...
        if exls:
            ...
            st.dataframe(de7.head(3),use_container_width=True)
