...
def fig_velas(df_p, nombre, freq, color="#00e676"):
    ...
    oh = pd.DataFrame({"open":open_,"high":high,"low":low,"close":close})
    # Si open es NaN (primer período), usar close como open
    oh["open"] = oh["open"].fillna(oh["close"])
    oh = oh.dropna(subset=["close","high","low"])
    if oh.empty: return None
    # Asegurar que high >= max(open,close) y low <= min(open,close)
    oh["high"] = oh[["high","open","close"]].max(axis=1)
    oh["low"]  = oh[["low","open","close"]].min(axis=1)
...
with t1:
    ...
    if len(dfp)<2:
        ...
        if fv:
            ...
            st.warning("No hay suficientes datos de precio para graficar velas. Verifica que el producto tiene precios cargados.")
        if crp:
